import pandas as pd
import aux
from especiales.ALIMENTACION_2.apps.paletas_ALIMENTACION_2 import paletas
from especiales.ALIMENTACION_2.apps.cuestionario_ALIMENTACION_2 import *
from especiales.ALIMENTACION_2.apps.txt_ALIMENTACION_2 import textos_ALIMENTACION_2
from especiales.ALIMENTACION_2.apps.files_ALIMENTACION_2 import files_ALIMENTACION_2
...
for pregunta, paleta in paletas.items():
    if pregunta not in preguntas_ocultas:
        cruces_dict[pregunta] = [textos_ALIMENTACION_2 ['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
        ...
        if pregunta not in preguntas_ocultas:
            ...
            ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='ALIMENTACION_2', textos=textos_ALIMENTACION_2,
                                                   files=files_ALIMENTACION_2, paletas=paletas, cruces_dict=cruces_dict,preguntas_ocultas=preguntas_ocultas,
                                                   opciones=tiene_opciones)


def dropdownTarget(id='selectTargetALIMENTACION_2', cantidadDeOpciones=20):
    opciones = [
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_ALIMENTACION_2', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
