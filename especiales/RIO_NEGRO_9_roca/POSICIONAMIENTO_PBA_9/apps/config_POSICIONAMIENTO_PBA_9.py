import pandas as pd
import aux
from especiales.POSICIONAMIENTO_PBA_9.apps.paletas_POSICIONAMIENTO_PBA_9 import paletas
from especiales.POSICIONAMIENTO_PBA_9.apps.cuestionario_POSICIONAMIENTO_PBA_9 import *
from especiales.POSICIONAMIENTO_PBA_9.apps.txt_POSICIONAMIENTO_PBA_9 import textos_POSICIONAMIENTO_PBA_9
from especiales.POSICIONAMIENTO_PBA_9.apps.files_POSICIONAMIENTO_PBA_9 import files_POSICIONAMIENTO_PBA_9
...
for pregunta, paleta in paletas.items():
    if pregunta not in preguntas_ocultas:
        ...
        cruces_dict[pregunta] = [textos_POSICIONAMIENTO_PBA_9['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
    ...
    ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='POSICIONAMIENTO_PBA_9', textos=textos_POSICIONAMIENTO_PBA_9,
                                           files=files_POSICIONAMIENTO_PBA_9, paletas=paletas, cruces_dict=cruces_dict, preguntas_ocultas=preguntas_ocultas,
...
def dropdownTarget(id='selectTargetPOSICIONAMIENTO_PBA_9', cantidadDeOpciones=17):
    opciones = [
        ...
        {'label': 'Región: GBA', 'value': 'GBA'},
        {'label': 'Región: Interior', 'value': 'Interior'},
...
def plot_cruces(candidato,df_cruces,files, name, preguntasQueNoVan, callbackInputTarget, crucesDict = crucesDict,opcionesDePreguntasInicial='', reorder=True):
    ...
    @app.callback(
        ...
        [Input('selectCruce' + name + candidato, 'value'),
         ...
         Input("selectOpciones_POSICIONAMIENTO_PBA_9", 'value'), ])
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_POSICIONAMIENTO_PBA_9', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
...
imagen = plots(files_POSICIONAMIENTO_PBA_9['csv_imagen'][0],files_POSICIONAMIENTO_PBA_9['csv_imagen'][3], files_POSICIONAMIENTO_PBA_9["Alberto Fernandez"][0], callbackInputTarget= 'selectTargetPOSICIONAMIENTO_PBA_9',opcionesDePreguntasInicial = 4,
                            preguntasQueNoVan= preguntas_ocultas, name='POSICIONAMIENTO_PBA_9-Comparativo',crucesDict = cruces_dict, files=files_POSICIONAMIENTO_PBA_9, tracking='POSICIONAMIENTO_PBA_9'),
...
ps['imagen1'] = html.Div([
                ...
                getStackedBarplot(files_POSICIONAMIENTO_PBA_9['csv_imagen'][0], 'selectTargetPOSICIONAMIENTO_PBA_9', 'selectOpciones_POSICIONAMIENTO_PBA_9', opinionColorDict, 'POSICIONAMIENTO_PBA_9-1D1A', opcionesDePreguntasInicial = 4),
...
ps['imagen2'] = html.Div([
                html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
                html.H5(textos['seleccioneFigura']),
                getStackedBarplot(files_POSICIONAMIENTO_PBA_9['csv_imagen'][1], 'selectTargetPOSICIONAMIENTO_PBA_9', 'selectOpciones_POSICIONAMIENTO_PBA_9', opinionColorDict, 'POSICIONAMIENTO_PBA_9-1D1B', opcionesDePreguntasInicial = 4),
                html.H3('Seleccione un dirigente o funcionario político'),
                imagen[0]])
...
ps['imagen3'] = html.Div([
                html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
                html.H5(textos['seleccioneFigura']),
                getStackedBarplot(files_POSICIONAMIENTO_PBA_9['csv_imagen'][2], 'selectTargetPOSICIONAMIENTO_PBA_9', 'selectOpciones_POSICIONAMIENTO_PBA_9', opinionColorDict, 'POSICIONAMIENTO_PBA_9-1D1C', opcionesDePreguntasInicial = 4),
                html.H3('Seleccione un dirigente o funcionario político'),
                imagen[0]])
