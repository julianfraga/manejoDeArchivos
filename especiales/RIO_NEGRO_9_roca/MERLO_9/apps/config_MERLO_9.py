import pandas as pd
import aux
from especiales.MERLO_9.apps.paletas_MERLO_9 import paletas
from especiales.MERLO_9.apps.cuestionario_MERLO_9 import *
from especiales.MERLO_9.apps.txt_MERLO_9 import textos_MERLO_9
from especiales.MERLO_9.apps.files_MERLO_9 import files_MERLO_9
...
for pregunta, paleta in paletas.items():
    if pregunta not in preguntas_ocultas:
        cruces_dict[pregunta] = [textos_MERLO_9 ['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
        ...
        if pregunta not in preguntas_ocultas:
            ...
            ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='MERLO_9', textos=textos_MERLO_9,
                                                   files=files_MERLO_9, paletas=paletas, cruces_dict=cruces_dict,preguntas_ocultas=preguntas_ocultas,
                                                   opciones=tiene_opciones)


def dropdownTarget(id='selectTargetMERLO_9', cantidadDeOpciones=17):
    opciones = [
...
def plot_cruces(candidato,df_cruces,files, name, preguntasQueNoVan, callbackInputTarget, crucesDict = crucesDict,opcionesDePreguntasInicial='', reorder=True):
    ...
    @app.callback(
        ...
        [Input('selectCruce' + name + candidato, 'value'),
         ...
         Input("selectOpciones_MERLO_9", 'value'), ])
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_MERLO_9', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
...
imagen = plots(files_MERLO_9['csv_imagen'][0],files_MERLO_9['csv_imagen'][3], files_MERLO_9['Alberto Fernández'][0], callbackInputTarget= 'selectTargetMERLO_9',opcionesDePreguntasInicial = 4,
                          preguntasQueNoVan= preguntas_ocultas, name='MERLO_9-Comparativo',crucesDict = cruces_dict, files=files_MERLO_9, tracking='MERLO_9'),
ps['imagen1'] = html.Div([
               html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
               html.H5(textos['seleccioneFigura']),
               getStackedBarplot(files_MERLO_9['csv_imagen'][0], 'selectTargetMERLO_9', 'selectOpciones_MERLO_9', opinionColorDict, 'MERLO_9-1D1A', opcionesDePreguntasInicial = 4),
...
ps['imagen2'] = html.Div([
               html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
               html.H5(textos['seleccioneFigura']),
               getStackedBarplot(files_MERLO_9['csv_imagen'][1], 'selectTargetMERLO_9', 'selectOpciones_MERLO_9', opinionColorDict, 'MERLO_9-1D1B', opcionesDePreguntasInicial = 4),
               html.H3('Seleccione un dirigente o funcionario político'),
               imagen[0]])
...
ps['imagen3'] = html.Div([
               html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
               html.H5(textos['seleccioneFigura']),
               getStackedBarplot(files_MERLO_9['csv_imagen'][2], 'selectTargetMERLO_9', 'selectOpciones_MERLO_9', opinionColorDict, 'MERLO_9-1D1C', opcionesDePreguntasInicial = 4),
               html.H3('Seleccione un dirigente o funcionario político'),
               imagen[0]])
