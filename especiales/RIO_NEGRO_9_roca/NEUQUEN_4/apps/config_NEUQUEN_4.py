import pandas as pd
import aux
from especiales.NEUQUEN_4.apps.paletas_NEUQUEN_4 import paletas
from especiales.NEUQUEN_4.apps.cuestionario_NEUQUEN_4 import *
from especiales.NEUQUEN_4.apps.txt_NEUQUEN_4 import textos_NEUQUEN_4
from especiales.NEUQUEN_4.apps.files_NEUQUEN_4 import files_NEUQUEN_4
...
for pregunta, paleta in paletas.items():
    if pregunta not in preguntas_ocultas:
        cruces_dict[pregunta] = [textos_NEUQUEN_4 ['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
        ...
        if pregunta not in preguntas_ocultas:
            ...
            ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='NEUQUEN_4', textos=textos_NEUQUEN_4,
                                                   files=files_NEUQUEN_4, paletas=paletas, cruces_dict=cruces_dict,preguntas_ocultas=preguntas_ocultas,
                                                   opciones=tiene_opciones)


def dropdownTarget(id='selectTargetNEUQUEN_4', cantidadDeOpciones=17):
    opciones = [
        ...
        {'label': 'Voto marzo 2019: Marcos Koopmam', 'value': 'Koopmam'},
        {'label': 'Voto marzo 2019: Ramón Rioseco', 'value': 'Rioseco'},
        {'label': 'Voto marzo 2019: Pablo Cervi', 'value': 'Cervi'},
        {'label': 'Voto marzo 2019: Carlos Eguía', 'value': 'Eguía'},
        {'label': 'Voto marzo 2019: Patricia Jure', 'value': 'Jure'}
        ]
...
def plot_cruces(candidato,df_cruces,files, name, preguntasQueNoVan, callbackInputTarget, crucesDict = crucesDict,opcionesDePreguntasInicial='', reorder=True):
    ...
    @app.callback(
        ...
        [Input('selectCruce' + name + candidato, 'value'),
         ...
         Input("selectOpciones_NEUQUEN_4", 'value'), ])
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_NEUQUEN_4', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
...
imagen = plots(files_NEUQUEN_4['csv_imagen'][0],files_NEUQUEN_4['csv_imagen'][3], files_NEUQUEN_4["Rolo Figueroa"][0], callbackInputTarget= 'selectTargetNEUQUEN_4',opcionesDePreguntasInicial = 4,
                            preguntasQueNoVan= preguntas_ocultas, name='NEUQUEN_4-Comparativo',crucesDict = cruces_dict, files=files_NEUQUEN_4, tracking='NEUQUEN_4'),
...
ps['imagen1'] = html.Div([
                ...
                getStackedBarplot(files_NEUQUEN_4['csv_imagen'][0], 'selectTargetNEUQUEN_4', 'selectOpciones_NEUQUEN_4', opinionColorDict, 'NEUQUEN_4-1D1A', opcionesDePreguntasInicial = 4),
...
ps['imagen2'] = html.Div([
                html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
                html.H5(textos['seleccioneFigura']),
                getStackedBarplot(files_NEUQUEN_4['csv_imagen'][1], 'selectTargetNEUQUEN_4', 'selectOpciones_NEUQUEN_4', opinionColorDict, 'NEUQUEN_4-1D1B', opcionesDePreguntasInicial = 4),
                html.H3('Seleccione un dirigente o funcionario político'),
                imagen[0]])
...
ps['imagen3'] = html.Div([
                html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
                html.H5(textos['seleccioneFigura']),
                getStackedBarplot(files_NEUQUEN_4['csv_imagen'][2], 'selectTargetNEUQUEN_4', 'selectOpciones_NEUQUEN_4', opinionColorDict, 'NEUQUEN_4-1D1C', opcionesDePreguntasInicial = 4),
                html.H3('Seleccione un dirigente o funcionario político'),
                imagen[0]])
