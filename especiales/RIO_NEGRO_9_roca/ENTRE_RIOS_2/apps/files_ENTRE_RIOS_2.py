from especiales.ENTRE_RIOS_2.apps.encuesta_ENTRE_RIOS_2 import encuesta, path, opciones
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_ENTRE_RIOS_2["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

# ruta de los archivos de imagenes
files_ENTRE_RIOS_2['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
# { 'Sergio Massa': ['./especiales/ENTRE_RIOS_2/data/cruce_pregunta_P61_4opc.csv',
#                   './especiales/ENTRE_RIOS_2/data/cruce_pregunta_P61_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_ENTRE_RIOS_2[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
