from especiales.TRACKING_DIARIO_CABA.apps.encuesta_TRACKING_DIARIO_CABA import encuesta, path, opciones

cuestionario = encuesta.cuestionario
files_TRACKING_DIARIO_CABA = {}
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_TRACKING_DIARIO_CABA["csv" + pregunta] = [(f'{data_folder}pregunta_' + pregunta + '.csv'),
                                      (f'{data_folder}serie_' + pregunta + '.csv'),
                                      (f'{data_folder}cruce_pregunta_' + pregunta + '.csv')]

# ruta de los archivos de imagenes
files_TRACKING_DIARIO_CABA['csv_imagen'] = [f'{data_folder}imagen_orden_positiva.csv',
...
# { 'Sergio Massa': ['./especiales/TRACKING_DIARIO_CABA/data/cruce_pregunta_P61_4opc.csv',
#                   './especiales/TRACKING_DIARIO_CABA/data/cruce_pregunta_P61_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_TRACKING_DIARIO_CABA[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
