from especiales.TRACKING_CABA_B.apps.encuesta_TRACKING_CABA_B import encuesta, path, opciones

cuestionario = encuesta.cuestionario
files_TRACKING_CABA_B = {}
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_TRACKING_CABA_B["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

# ruta de los archivos de imagenes
files_TRACKING_CABA_B['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
# { 'Sergio Massa': ['./especiales/TRACKING_CABA_B/data/cruce_pregunta_P61_4opc.csv',
#                   './especiales/TRACKING_CABA_B/data/cruce_pregunta_P61_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_TRACKING_CABA_B[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
