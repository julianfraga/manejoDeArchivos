from especiales.MENDOZA_1.apps.cuestionario_MENDOZA_1 import cuestionario, path, opciones

files_MENDOZA_1 = {}
data_folder = path + "data/"

for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_MENDOZA_1["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

files_MENDOZA_1['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_MENDOZA_1[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
