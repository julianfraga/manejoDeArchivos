from especiales.RIO_NEGRO_7_roca.apps.encuesta_RIO_NEGRO_7_roca import encuesta, path, opciones

cuestionario = encuesta.cuestionario
files_RIO_NEGRO_7_roca = {}
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_RIO_NEGRO_7_roca["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

# ruta de los archivos de imagenes
files_RIO_NEGRO_7_roca['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
# { 'Sergio Massa': ['./especiales/RIO_NEGRO_7_roca/data/cruce_pregunta_P61_4opc.csv',
#                   './especiales/RIO_NEGRO_7_roca/data/cruce_pregunta_P61_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_RIO_NEGRO_7_roca[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
