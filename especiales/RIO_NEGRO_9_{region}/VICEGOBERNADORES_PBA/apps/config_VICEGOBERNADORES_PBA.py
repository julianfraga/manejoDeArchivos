import pandas as pd
import aux
from especiales.VICEGOBERNADORES_PBA.apps.paletas_VICEGOBERNADORES_PBA import paletas
from especiales.VICEGOBERNADORES_PBA.apps.cuestionario_VICEGOBERNADORES_PBA import *
from especiales.VICEGOBERNADORES_PBA.apps.txt_VICEGOBERNADORES_PBA import textos_VICEGOBERNADORES_PBA
from especiales.VICEGOBERNADORES_PBA.apps.files_VICEGOBERNADORES_PBA import files_VICEGOBERNADORES_PBA
...
for pregunta, paleta in paletas.items():
    if pregunta not in preguntas_ocultas:
        cruces_dict[pregunta] = [textos_VICEGOBERNADORES_PBA ['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
        ...
        if pregunta not in preguntas_ocultas:
            ...
            ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='VICEGOBERNADORES_PBA', textos=textos_VICEGOBERNADORES_PBA,
                                                   files=files_VICEGOBERNADORES_PBA, paletas=paletas, cruces_dict=cruces_dict,preguntas_ocultas=preguntas_ocultas,
                                                   opciones=tiene_opciones)


def dropdownTarget(id='selectTargetVICEGOBERNADORES_PBA', cantidadDeOpciones=17):
    opciones = [
        ...
        {'label': 'Interior de PBA', 'value': 'Interior'},
...
def plot_cruces(candidato,df_cruces,files, name, preguntasQueNoVan, callbackInputTarget, crucesDict = crucesDict,opcionesDePreguntasInicial='', reorder=True):
    ...
    @app.callback(
        ...
        [Input('selectCruce' + name + candidato, 'value'),
         ...
         Input("selectOpciones_VICEGOBERNADORES_PBA", 'value'), ])
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_VICEGOBERNADORES_PBA', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
