import pandas as pd
import aux
from especiales.SANTA_CRUZ_1.apps.paletas_SANTA_CRUZ_1 import paletas
from especiales.SANTA_CRUZ_1.apps.cuestionario_SANTA_CRUZ_1 import *
from especiales.SANTA_CRUZ_1.apps.txt_SANTA_CRUZ_1 import textos_SANTA_CRUZ_1
from especiales.SANTA_CRUZ_1.apps.files_SANTA_CRUZ_1 import files_SANTA_CRUZ_1
...
for pregunta, paleta in paletas.items():
    ...
    cruces_dict[pregunta] = [textos_SANTA_CRUZ_1['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
    ...
    ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='SANTA_CRUZ_1', textos=textos_SANTA_CRUZ_1,
                                           files=files_SANTA_CRUZ_1, paletas=paletas, cruces_dict=cruces_dict,
...
def dropdownTarget(id='selectTargetSANTA_CRUZ_1', cantidadDeOpciones=17):
    opciones = [
        ...
        {'label': 'Voto 2019: A. Kirchner', 'value': 'A.Kirchner'},
        {'label': 'Voto 2019: Belloni', 'value': 'Belloni'},
        {'label': 'Voto 2019: Vidal', 'value': 'Vidal'},
        {'label': 'Voto 2019: Costa', 'value': 'Costa'},
        {'label': 'Voto 2019: Otro Nueva Santa Cruz', 'value': 'Otro Nueva Santa Cruz'},
        {'label': 'Voto 2019: Peralta', 'value': 'Peralta'},
...
def plot_cruces(candidato,df_cruces,files, name, preguntasQueNoVan, callbackInputTarget, crucesDict = crucesDict,opcionesDePreguntasInicial='', reorder=True):
    ...
    @app.callback(
        ...
        [Input('selectCruce' + name + candidato, 'value'),
         ...
         Input("selectOpciones_SANTA_CRUZ_1", 'value'), ])
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_SANTA_CRUZ_1', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
...
imagen = plots(files_SANTA_CRUZ_1['csv_imagen'][0],files_SANTA_CRUZ_1['csv_imagen'][3], files_SANTA_CRUZ_1["Pablo González"][0], callbackInputTarget= 'selectTargetSANTA_CRUZ_1',opcionesDePreguntasInicial = 4,
                            preguntasQueNoVan= preguntas_ocultas, name='SANTA_CRUZ_1-Comparativo',crucesDict = cruces_dict, files=files_SANTA_CRUZ_1, tracking='SANTA_CRUZ_1'),
...
ps['imagen1'] = html.Div([
                ...
                getStackedBarplot(files_SANTA_CRUZ_1['csv_imagen'][0], 'selectTargetSANTA_CRUZ_1', 'selectOpciones_SANTA_CRUZ_1', opinionColorDict, 'SANTA_CRUZ_1-1D1A', opcionesDePreguntasInicial = 4),
...
ps['imagen2'] = html.Div([
                html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
                html.H5(textos['seleccioneFigura']),
                getStackedBarplot(files_SANTA_CRUZ_1['csv_imagen'][1], 'selectTargetSANTA_CRUZ_1', 'selectOpciones_SANTA_CRUZ_1', opinionColorDict, 'SANTA_CRUZ_1-1D1B', opcionesDePreguntasInicial = 4),
                html.H3('Seleccione un dirigente o funcionario político'),
                imagen[0]])
...
ps['imagen3'] = html.Div([
                html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
                html.H5(textos['seleccioneFigura']),
                getStackedBarplot(files_SANTA_CRUZ_1['csv_imagen'][2], 'selectTargetSANTA_CRUZ_1', 'selectOpciones_SANTA_CRUZ_1', opinionColorDict, 'SANTA_CRUZ_1-1D1C', opcionesDePreguntasInicial = 4),
                html.H3('Seleccione un dirigente o funcionario político'),
                imagen[0]])
