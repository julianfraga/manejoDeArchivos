import pandas as pd
import aux
from especiales.TRANSPORTE_1.apps.paletas_TRANSPORTE_1 import paletas
from especiales.TRANSPORTE_1.apps.cuestionario_TRANSPORTE_1 import *
from especiales.TRANSPORTE_1.apps.txt_TRANSPORTE_1 import textos_TRANSPORTE_1
from especiales.TRANSPORTE_1.apps.files_TRANSPORTE_1 import files_TRANSPORTE_1
...
for pregunta, paleta in paletas.items():
    if pregunta not in preguntas_ocultas:
        cruces_dict[pregunta] = [textos_TRANSPORTE_1 ['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
        ...
        if pregunta not in preguntas_ocultas:
            ...
            ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='TRANSPORTE_1', textos=textos_TRANSPORTE_1,
                                                   files=files_TRANSPORTE_1, paletas=paletas, cruces_dict=cruces_dict,preguntas_ocultas=preguntas_ocultas,
                                                   opciones=tiene_opciones)


def dropdownTarget(id='selectTargetTRANSPORTE_1', cantidadDeOpciones=20):
    opciones = [
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_TRANSPORTE_1', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
