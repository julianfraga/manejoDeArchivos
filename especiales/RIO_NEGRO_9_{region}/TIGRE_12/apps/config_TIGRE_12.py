import pandas as pd
import aux
from especiales.TIGRE_12.apps.paletas_TIGRE_12 import paletas
from especiales.TIGRE_12.apps.cuestionario_TIGRE_12 import *
from especiales.TIGRE_12.apps.txt_TIGRE_12 import textos_TIGRE_12
from especiales.TIGRE_12.apps.files_TIGRE_12 import files_TIGRE_12
...
for pregunta, paleta in paletas.items():
    if pregunta not in preguntas_ocultas:
        cruces_dict[pregunta] = [textos_TIGRE_12 ['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
        ...
        if pregunta not in preguntas_ocultas:
            ...
            ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='TIGRE_12', textos=textos_TIGRE_12,
                                                   files=files_TIGRE_12, paletas=paletas, cruces_dict=cruces_dict,preguntas_ocultas=preguntas_ocultas,
                                                   opciones=tiene_opciones)


def dropdownTarget(id='selectTargetTIGRE_12', cantidadDeOpciones=17):
    opciones = [
...
def plot_cruces(candidato,df_cruces,files, name, preguntasQueNoVan, callbackInputTarget, crucesDict = crucesDict,opcionesDePreguntasInicial='', reorder=True):
    ...
    @app.callback(
        ...
        [Input('selectCruce' + name + candidato, 'value'),
         ...
         Input("selectOpciones_TIGRE_12", 'value'), ])
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_TIGRE_12', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
...
imagen = plots(files_TIGRE_12['csv_imagen'][0],files_TIGRE_12['csv_imagen'][3], files_TIGRE_12['Nicolás Massot'][0], callbackInputTarget= 'selectTargetTIGRE_12',opcionesDePreguntasInicial = 4,
                          preguntasQueNoVan= preguntas_ocultas, name='TIGRE_12-Comparativo',crucesDict = cruces_dict, files=files_TIGRE_12, tracking='TIGRE_12'),
ps['imagen1'] = html.Div([
               html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
               html.H5(textos['seleccioneFigura']),
               getStackedBarplot(files_TIGRE_12['csv_imagen'][0], 'selectTargetTIGRE_12', 'selectOpciones_TIGRE_12', opinionColorDict, 'TIGRE_12-1D1A', opcionesDePreguntasInicial = 4),
...
ps['imagen2'] = html.Div([
               html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
               html.H5(textos['seleccioneFigura']),
               getStackedBarplot(files_TIGRE_12['csv_imagen'][1], 'selectTargetTIGRE_12', 'selectOpciones_TIGRE_12', opinionColorDict, 'TIGRE_12-1D1B', opcionesDePreguntasInicial = 4),
               html.H3('Seleccione un dirigente o funcionario político'),
               imagen[0]])
...
ps['imagen3'] = html.Div([
               html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
               html.H5(textos['seleccioneFigura']),
               getStackedBarplot(files_TIGRE_12['csv_imagen'][2], 'selectTargetTIGRE_12', 'selectOpciones_TIGRE_12', opinionColorDict, 'TIGRE_12-1D1C', opcionesDePreguntasInicial = 4),
               html.H3('Seleccione un dirigente o funcionario político'),
               imagen[0]])
