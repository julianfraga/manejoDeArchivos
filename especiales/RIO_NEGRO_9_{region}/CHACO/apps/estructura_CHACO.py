...
from especiales.CHACO.apps.paletas_CHACO import paletas
from especiales.CHACO.apps.files_CHACO import files_CHACO
...
for _,row in encuesta.cuestionario[~encuesta.cuestionario.codigo.isin(encuesta.preguntas_imagenes)].iterrows():
    ...
    layout_preguntas[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='CHACO', textos=encuesta.get_textos,
                                           files=files_CHACO, paletas=paletas, cruces_dict=cruces_dict, preguntas_ocultas=encuesta.preguntas_ocultas,
...
if encuesta.preguntas_imagenes:
    ...
    imagen = plots(files_CHACO['csv_imagen'][0],files_CHACO['csv_imagen'][3], files_CHACO["Sergio Massa"][0], callbackInputTarget= 'selectTargetCHACO',opcionesDePreguntasInicial = 4,
                                preguntasQueNoVan= encuesta.preguntas_ocultas, name='CHACO-Comparativo',crucesDict = cruces_dict, files=files_CHACO, codigo='CHACO')
    ...
    layout_preguntas['imagen1'] = html.Div([
                    ...
                    getStackedBarplot(files_CHACO['csv_imagen'][0], 'selectTargetCHACO', 'selectOpciones_CHACO',
                                      opinionColorDict, 'CHACO-1D1A', opcionesDePreguntasInicial = 4), # Grafico comparado positiva
    ...
    layout_preguntas['imagen2'] = html.Div([
                    html.H3(textos_base['pregunta_imagen'], className='subtituloBloque'),
                    html.H5(textos_base['seleccioneFigura']),
                    getStackedBarplot(files_CHACO['csv_imagen'][1], 'selectTargetCHACO', 'selectOpciones_CHACO',
                                      opinionColorDict, 'CHACO-1D1B', opcionesDePreguntasInicial = 4), # Grafico compardo Negativa
                    html.H3('Seleccione un dirigente o funcionario político'),
                    imagen])
    ...
    layout_preguntas['imagen3'] = html.Div([
                    html.H3(textos_base['pregunta_imagen'], className='subtituloBloque'),
                    html.H5(textos_base['seleccioneFigura']),
                    getStackedBarplot(files_CHACO['csv_imagen'][2], 'selectTargetCHACO', 'selectOpciones_CHACO',
                                      opinionColorDict, 'CHACO-1D1C', opcionesDePreguntasInicial = 4), # Grafico comparado Ratio
                    html.H3('Seleccione un dirigente o funcionario político'),
                    imagen])