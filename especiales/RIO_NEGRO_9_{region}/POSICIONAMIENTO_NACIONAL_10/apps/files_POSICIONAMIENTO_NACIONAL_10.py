from especiales.POSICIONAMIENTO_NACIONAL_10.apps.cuestionario_POSICIONAMIENTO_NACIONAL_10 import cuestionario, path, opciones

files_POSICIONAMIENTO_NACIONAL_10 = {}
data_folder = path + "data/"

for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_POSICIONAMIENTO_NACIONAL_10["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

files_POSICIONAMIENTO_NACIONAL_10['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_POSICIONAMIENTO_NACIONAL_10[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
