from especiales.TRACKING_DIARIO_NACIONAL.apps.encuesta_TRACKING_DIARIO_NACIONAL import encuesta, path, opciones

cuestionario = encuesta.cuestionario
files_TRACKING_DIARIO_NACIONAL = {}
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_TRACKING_DIARIO_NACIONAL["csv" + pregunta] = [(f'{data_folder}pregunta_' + pregunta + '.csv'),
                                      (f'{data_folder}serie_' + pregunta + '.csv'),
                                      (f'{data_folder}cruce_pregunta_' + pregunta + '.csv')]

# ruta de los archivos de imagenes
files_TRACKING_DIARIO_NACIONAL['csv_imagen'] = [f'{data_folder}imagen_orden_positiva.csv',
                           f'{data_folder}imagen_orden_ratio.csv',
                           f'{data_folder}imagen_orden_negativa.csv',
                           f'{data_folder}serie_imagen.csv'
                              ]
...
# { 'Sergio Massa': ['./especiales/TRACKING_DIARIO_NACIONAL/data/cruce_pregunta_P61_4opc.csv',
#                   './especiales/TRACKING_DIARIO_NACIONAL/data/cruce_pregunta_P61_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_TRACKING_DIARIO_NACIONAL[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
