from especiales.TRACKING_PBA_ENTRESEMANA.apps.encuesta_TRACKING_PBA_ENTRESEMANA import encuesta, path, opciones

cuestionario = encuesta.cuestionario
files_TRACKING_PBA_ENTRESEMANA = {}
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_TRACKING_PBA_ENTRESEMANA["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

# ruta de los archivos de imagenes
files_TRACKING_PBA_ENTRESEMANA['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
# { 'Sergio Massa': ['./especiales/TRACKING_PBA_ENTRESEMANA/data/cruce_pregunta_P63_4opc.csv',
#                   './especiales/TRACKING_PBA_ENTRESEMANA/data/cruce_pregunta_P63_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_TRACKING_PBA_ENTRESEMANA[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
