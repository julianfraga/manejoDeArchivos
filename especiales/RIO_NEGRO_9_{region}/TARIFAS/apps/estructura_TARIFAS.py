...
from especiales.TARIFAS.apps.paletas_TARIFAS import paletas
from especiales.TARIFAS.apps.files_TARIFAS import files_TARIFAS
...
for _,row in encuesta.cuestionario[~encuesta.cuestionario.codigo.isin(encuesta.preguntas_imagenes)].iterrows():
    ...
    layout_preguntas[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='TARIFAS', textos=encuesta.get_textos,
                                           files=files_TARIFAS, paletas=paletas, cruces_dict=cruces_dict, preguntas_ocultas=encuesta.preguntas_ocultas,
...
if encuesta.preguntas_imagenes:
    ...
    imagen = plots(files_TARIFAS['csv_imagen'][0],files_TARIFAS['csv_imagen'][3], files_TARIFAS["Sergio Massa"][0], callbackInputTarget= 'selectTargetTARIFAS',opcionesDePreguntasInicial = 4,
                                preguntasQueNoVan= encuesta.preguntas_ocultas, name='TARIFAS-Comparativo',crucesDict = cruces_dict, files=files_TARIFAS, codigo='TARIFAS')
    ...
    layout_preguntas['imagen1'] = html.Div([
                    ...
                    getStackedBarplot(files_TARIFAS['csv_imagen'][0], 'selectTargetTARIFAS', 'selectOpciones_TARIFAS',
                                      opinionColorDict, 'TARIFAS-1D1A', opcionesDePreguntasInicial = 4), # Grafico comparado positiva
    ...
    layout_preguntas['imagen2'] = html.Div([
                    html.H3(textos_base['pregunta_imagen'], className='subtituloBloque'),
                    html.H5(textos_base['seleccioneFigura']),
                    getStackedBarplot(files_TARIFAS['csv_imagen'][1], 'selectTargetTARIFAS', 'selectOpciones_TARIFAS',
                                      opinionColorDict, 'TARIFAS-1D1B', opcionesDePreguntasInicial = 4), # Grafico compardo Negativa
                    html.H3('Seleccione un dirigente o funcionario político'),
                    imagen])
    ...
    layout_preguntas['imagen3'] = html.Div([
                    html.H3(textos_base['pregunta_imagen'], className='subtituloBloque'),
                    html.H5(textos_base['seleccioneFigura']),
                    getStackedBarplot(files_TARIFAS['csv_imagen'][2], 'selectTargetTARIFAS', 'selectOpciones_TARIFAS',
                                      opinionColorDict, 'TARIFAS-1D1C', opcionesDePreguntasInicial = 4), # Grafico comparado Ratio
                    html.H3('Seleccione un dirigente o funcionario político'),
                    imagen])