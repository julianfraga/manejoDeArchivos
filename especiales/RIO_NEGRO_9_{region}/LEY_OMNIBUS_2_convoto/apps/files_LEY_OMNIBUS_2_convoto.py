from especiales.LEY_OMNIBUS_2_convoto.apps.encuesta_LEY_OMNIBUS_2_convoto import encuesta, path, opciones
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_LEY_OMNIBUS["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

# ruta de los archivos de imagenes
files_LEY_OMNIBUS['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
# { 'Sergio Massa': ['./especiales/LEY_OMNIBUS_2_convoto/data/cruce_pregunta_P61_4opc.csv',
#                   './especiales/LEY_OMNIBUS_2_convoto/data/cruce_pregunta_P61_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_LEY_OMNIBUS[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
