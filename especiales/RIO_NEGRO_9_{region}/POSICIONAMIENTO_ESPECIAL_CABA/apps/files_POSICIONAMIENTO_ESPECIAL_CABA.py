from especiales.POSICIONAMIENTO_ESPECIAL_CABA.apps.encuesta_POSICIONAMIENTO_ESPECIAL_CABA import encuesta, path, opciones

cuestionario = encuesta.cuestionario
files_POSICIONAMIENTO_ESPECIAL_CABA = {}
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_POSICIONAMIENTO_ESPECIAL_CABA["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

# ruta de los archivos de imagenes
files_POSICIONAMIENTO_ESPECIAL_CABA['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
# { 'Sergio Massa': ['./especiales/POSICIONAMIENTO_ESPECIAL_CABA/data/cruce_pregunta_P61_4opc.csv',
#                   './especiales/POSICIONAMIENTO_ESPECIAL_CABA/data/cruce_pregunta_P61_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_POSICIONAMIENTO_ESPECIAL_CABA[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
