from especiales.GRUPOS_SCHIARETTI.apps.encuesta_GRUPOS_SCHIARETTI import encuesta, path, opciones

cuestionario = encuesta.cuestionario
files_GRUPOS_SCHIARETTI = {}
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_GRUPOS_SCHIARETTI["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

# ruta de los archivos de imagenes
files_GRUPOS_SCHIARETTI['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
# { 'Sergio Massa': ['./especiales/GRUPOS_SCHIARETTI/data/cruce_pregunta_P61_4opc.csv',
#                   './especiales/GRUPOS_SCHIARETTI/data/cruce_pregunta_P61_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_GRUPOS_SCHIARETTI[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
