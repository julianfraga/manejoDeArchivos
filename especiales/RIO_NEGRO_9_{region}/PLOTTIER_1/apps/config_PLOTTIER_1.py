import pandas as pd
import aux
from especiales.PLOTTIER_1.apps.paletas_PLOTTIER_1 import paletas
from especiales.PLOTTIER_1.apps.cuestionario_PLOTTIER_1 import *
from especiales.PLOTTIER_1.apps.txt_PLOTTIER_1 import textos_PLOTTIER_1
from especiales.PLOTTIER_1.apps.files_PLOTTIER_1 import files_PLOTTIER_1
...
for pregunta, paleta in paletas.items():
    if pregunta not in preguntas_ocultas:
        cruces_dict[pregunta] = [textos_PLOTTIER_1 ['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
        ...
        if pregunta not in preguntas_ocultas:
            ...
            ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='PLOTTIER_1', textos=textos_PLOTTIER_1,
                                                   files=files_PLOTTIER_1, paletas=paletas, cruces_dict=cruces_dict,preguntas_ocultas=preguntas_ocultas,
                                                   opciones=tiene_opciones)


def dropdownTarget(id='selectTargetPLOTTIER_1', cantidadDeOpciones=17):
    opciones = [
...
def plot_cruces(candidato,df_cruces,files, name, preguntasQueNoVan, callbackInputTarget, crucesDict = crucesDict,opcionesDePreguntasInicial='', reorder=True):
    ...
    @app.callback(
        ...
        [Input('selectCruce' + name + candidato, 'value'),
         ...
         Input("selectOpciones_PLOTTIER_1", 'value'), ])
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_PLOTTIER_1', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
