import pandas as pd
import aux
from especiales.MORON_1.apps.paletas_MORON_1 import paletas
from especiales.MORON_1.apps.cuestionario_MORON_1 import *
from especiales.MORON_1.apps.txt_MORON_1 import textos_MORON_1
from especiales.MORON_1.apps.files_MORON_1 import files_MORON_1
...
for pregunta, paleta in paletas.items():
    if pregunta not in preguntas_ocultas:
        cruces_dict[pregunta] = [textos_MORON_1['label' + pregunta], paleta]
...
for _,row in cuestionario[~cuestionario.codigo.isin(preguntas_imagenes)].iterrows():
        ...
        if pregunta not in preguntas_ocultas:
            ...
            ps[pregunta] = aux.get_pregunta_layout(pregunta, encuesta='MORON_1', textos=textos_MORON_1,
                                                   files=files_MORON_1, paletas=paletas, cruces_dict=cruces_dict,preguntas_ocultas=preguntas_ocultas,
                                                   opciones=tiene_opciones)




def dropdownTarget(id='selectTargetMORON_1', cantidadDeOpciones=17):
    opciones = [
        ...
        {'label': 'Voto octubre 2019: Gui', 'value': 'Gui'},
        {'label': 'Voto octubre 2019: Tagliaferro', 'value': 'Tagliaferro'},
        {'label': 'Voto octubre 2019: Cassese', 'value': 'Cassese'},
        {'label': 'Voto octubre 2019: Sebriano', 'value': 'Sebriano'},
        {'label': 'Voto octubre 2019: F.Fernández', 'value': 'F. Fernández'},
...
def plot_cruces(candidato,df_cruces,files, name, preguntasQueNoVan, callbackInputTarget, crucesDict = crucesDict,opcionesDePreguntasInicial='', reorder=True):
    ...
    @app.callback(
        ...
        [Input('selectCruce' + name + candidato, 'value'),
         ...
         Input("selectOpciones_MORON_1", 'value'), ])
...
def plots(csv_hbp,csv_ts, csv_cruces, callbackInputTarget = '', crucesDict = crucesDict, opcionesDePreguntasInicial = 4, preguntasQueNoVan = "", preguntasDeshabilitadas = '', name = '', tracking='Nacional', files=''):
    ...
    for candidato in candidatos:
        ...
        hbarplot  = plot_hbarplot(csv_hbp, df_hbp, candidato, callbackInputTarget, 'selectOpciones_MORON_1', opinionColorDict, name, opcionesDePreguntasInicial = opcionesDePreguntasInicial)
...
imagen = plots(files_MORON_1['csv_imagen'][0],files_MORON_1['csv_imagen'][3], files_MORON_1['Alberto Fernández'][0], callbackInputTarget= 'selectTargetMORON_1',opcionesDePreguntasInicial = 4,
                            preguntasQueNoVan= preguntas_ocultas, name='MORON_1-Comparativo',crucesDict = cruces_dict, files=files_MORON_1, tracking='MORON_1'),
...
ps['imagen1'] = html.Div([
                ...
                getStackedBarplot(files_MORON_1['csv_imagen'][0], 'selectTargetMORON_1', 'selectOpciones_MORON_1', opinionColorDict, 'MORON_1-1D1A', opcionesDePreguntasInicial = 4),
...
ps['imagen2'] = html.Div([
                html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
                html.H5(textos['seleccioneFigura']),
                getStackedBarplot(files_MORON_1['csv_imagen'][1], 'selectTargetMORON_1', 'selectOpciones_MORON_1', opinionColorDict, 'MORON_1-1D1B', opcionesDePreguntasInicial = 4),
                html.H3('Seleccione un dirigente o funcionario político'),
                imagen[0]])
...
ps['imagen3'] = html.Div([
                html.H3(textos['pregunta_imagen'], className='subtituloBloque'),
                html.H5(textos['seleccioneFigura']),
                getStackedBarplot(files_MORON_1['csv_imagen'][2], 'selectTargetMORON_1', 'selectOpciones_MORON_1', opinionColorDict, 'MORON_1-1D1C', opcionesDePreguntasInicial = 4),
                html.H3('Seleccione un dirigente o funcionario político'),
                imagen[0]])
