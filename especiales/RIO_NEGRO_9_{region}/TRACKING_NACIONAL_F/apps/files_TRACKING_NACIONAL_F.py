from especiales.TRACKING_NACIONAL_F.apps.encuesta_TRACKING_NACIONAL_F import encuesta, path, opciones

cuestionario = encuesta.cuestionario
files_TRACKING_NACIONAL_F = {}
...
for pregunta in cuestionario.query('bloque != "imagen"').codigo:
    files_TRACKING_NACIONAL_F["csv" + pregunta] = [('{}pregunta_' + pregunta + '.csv').format(data_folder),
                                      ('{}serie_' + pregunta + '.csv').format(data_folder),
                                      ('{}cruce_pregunta_' + pregunta + '.csv').format(data_folder)]

# ruta de los archivos de imagenes
files_TRACKING_NACIONAL_F['csv_imagen'] = ['{}imagen_orden_positiva.csv'.format(data_folder),
...
# { 'Sergio Massa': ['./especiales/TRACKING_NACIONAL_F/data/cruce_pregunta_P61_4opc.csv',
#                   './especiales/TRACKING_NACIONAL_F/data/cruce_pregunta_P61_6opc.csv'], ...
...
for _, row in cuestionario.query('paleta == "imagen"').iterrows():
    ...
    files_TRACKING_NACIONAL_F[figura] = ['{}cruce_pregunta_{}{}.csv'.format(data_folder, row.codigo, opcion)
